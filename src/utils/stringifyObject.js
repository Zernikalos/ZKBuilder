...
export default function stringifyObject(input, options, pad) {
    ...
    return (function stringify(input, options = {}, pad = '') {
        ...
        if (ArrayBuffer.isView(input)) {
            ...
            const auxInput = Array.from(input)

            const returnValue = '[' + auxInput.map((element, i) => {
                // const eol = auxInput.length - 1 === i ? tokens.newline : ',' + tokens.newlineOrSpace;
                const eol = auxInput.length - 1 === i ? '' : ',';
        ...
        if (isObject(input)) {
            ...
            const returnValue = '{' + tokens.newline + objectKeys.map((element, i) => {
                ...
                let value = stringify(inputElement, options, pad + indent);
                if (options.transform) {
